...
public class pr6 {
    public static class Watch { //объявление суперкласса(класс родитель)
    ...
    public static void main(String[] args) {
        ...
        while (comm!=4){
            ...
            {
                case 1:{
                    for (int i=0;i<n;i++) if(eWatches[i].getPrice()>max) max = eWatches[i].getPrice();
                    for (int i=0;i<m;i++) if(mWatches[i].getPrice()>max) max = mWatches[i].getPrice();
